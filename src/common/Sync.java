...
public class Sync {
     ...
     * (Use only for debugging; prefer explicit or fluent waits in production.)
     */
    ...
    public void safeExplicitWait(String locator, int seconds) {
        ...
        try {
            ...
            Reporter.log(String.format("❌ Timeout: Element(s) not visible within %d sec for XPath: %s", seconds, locator), true);
        } catch (Exception e) {
    ...
    public void safeFluentWait(String locator, int maxTimeoutSeconds, int pollingEverySeconds) {
        try {
            ...
            Reporter.log(String.format("✅ Element visible with XPath: %s (Timeout: %d sec | Polling: %d sec)",
                    locator, maxTimeoutSeconds, pollingEverySeconds), true);
        } catch (TimeoutException e) {
            Reporter.log(String.format("❌ Timeout: Element not visible within %d sec for XPath: %s",
                    maxTimeoutSeconds, locator), true);
        } catch (Exception e) {
    ...
    /**
     * Waits until the element becomes selected (useful for checkboxes/radio buttons).
     */
    public void waitForElementToBeSelected(String locator, int seconds) {
        ...
        try {
            ...
            Reporter.log(String.format("❌ Timeout: Element not selected within %d sec for XPath: %s", seconds, locator), true);
        } catch (Exception e) {
            Reporter.log(String.format("❌ Error while waiting for selection of XPath %s: %s", locator, e.getMessage()), true);
        }
    }
    ...
    public void waitForElementToBeClickable(String locator, int seconds) {
        ...
        try {
            ...
            Reporter.log(String.format("❌ Timeout: Element not clickable within %d sec for XPath: %s", seconds, locator), true);
        } catch (Exception e) {
            Reporter.log(String.format("❌ Error while waiting for clickable XPath %s: %s", locator, e.getMessage()), true);
        }
    }
}
